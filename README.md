# 🌟 Sanelise Matanzima — Portfolio  

<img src="images/cover.jpg.png" alt="Portfolio Cover" width="100%"/>

**IT Support Associate | Psychology Graduate | Emerging AI Innovator**  

Welcome to my personal portfolio! I build ethical, human-centered AI solutions, bridging psychology insights with technical expertise to create meaningful experiences.  

---

## 👩‍💻 About Me  

<table>
<tr>
<td>
I am <strong>Sanelise Matanzima</strong>, a Psychology graduate and emerging AI innovator dedicated to designing solutions that balance human insight with technological innovation.<br><br>

Passionate about interdisciplinary problem-solving, I bring creativity, resilience, and technical expertise to every challenge.  
My work focuses on developing platforms and applications that are inclusive, user-centric, and aligned with ethical standards.<br><br>

Thriving at the intersection of psychology and technology, I strive to deliver solutions that are not only technically robust but also genuinely human-centered.
</td>
<td>
<img src="images/profile.jpg.png" alt="Sanelise Profile" width="200" style="object-fit: cover;"/>
</td>
</tr>
</table>

---

## 🎯 Career Objective  
Seeking opportunities in **AI, data analytics, or intelligent system development**, particularly in career development, education, or health tech.  

---

## 🛠️ My Approach  
- **Human-Centered Design** – Using psychology principles to create intuitive and accessible solutions.  
- **Technical Excellence** – Building robust applications with modern tools and best practices.  
- **Ethical AI** – Committed to developing fair, transparent, and beneficial AI systems.  

---

## 🚀 Projects  

### 1. IT Master Chatbot  
<img src="images/it-master-chatbot.png" alt="IT Master Chatbot" width="400"/>  

Conversational AI learning companion for beginners, simplifying AI concepts with custom learning paths.  
**Tech Stack:** Botpress, NLP, Conversation Design  
🔗 [Project Page](#) | [Live Demo](#)  

---

### 2. Career Canvas Smart Build  
<img src="images/career-canvas.png" alt="Career Canvas Smart Build" width="400"/>  

Professional document platform with modular templates and responsive design for cross-device use.  
**Tech Stack:** React, TypeScript, Tailwind CSS  
🔗 [Project Page](https://github.com/Sanelise01/career-canvas-smart-build) | [Live Demo](https://career-canvas-smart-build.vercel.app)  

---

### 3. CreatiVerseAI  
<img src="images/creativerseai.png.png" alt="CreatiVerseAI" width="400"/>  

AI-powered creative writing tool generating poems, stories, and literary prompts using LLaMA 3.3 and Groq API.  
**Tech Stack:** Groq API, LLaMA 3.3, Python, Streamlit  
🔗 [Project Page](#) | [Live Demo](#)  

---

### 4. Fairness Audit Wizard  
<img src="images/fairness-audit.png.png" alt="Fairness Audit Wizard" width="400"/>  

Analytical tool for bias detection in ML models, with dashboards and mitigation insights.  
**Tech Stack:** Python, Pandas, Scikit-learn, Plotly  
🔗 [Project Page](https://github.com/Sanelise01/fairness-unveiled-insights) | [Live Demo](#)  

---

### 5. Machine Daily Check Prototype  
<img src="images/machine-daily-check.png.png" alt="Machine Daily Check Prototype" width="400"/>  

Tablet-friendly UX/UI prototype streamlining industrial machine maintenance checks.  
**Tech Stack:** Figma, UX Design, Prototyping  
🔗 [Project Page](https://www.figma.com/design/4l9za9IJtBn06GS4tPwqfj/Machine-Daily-Check_Prototype) | [Live Demo](#)  

---

## 🧰 Technical Skills  
- **Programming & AI/ML:** Python, OpenAI API, Supabase, Botpress, Flowise AI  
- **Design & Prototyping:** Figma, Human-Centered Design  
- **Database & Version Control:** SQL, Git  

**Core Competencies:**  
Problem-solving • Collaboration • Communication • Presentation • Critical Thinking • Adaptability  

---

## 📬 Contact Me  
📧 Email: [sanelisematanzima@gmail.com](mailto:sanelisematanzima@gmail.com)  
🐙 GitHub: [Sanelise01](https://github.com/Sanelise01)  
💼 LinkedIn: [Sanelise Matanzima](https://linkedin.com/in/sanelise-matanzima)  
🌍 Portfolio Live: [Click Here](#)  
